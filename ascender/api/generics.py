...
from ascender.dab.lib.utils.models import get_all_field_names
...
class LoggedLoginView(auth_views.LoginView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            ...
            ret.setdefault('X-API-Session-Cookie-Name', getattr(settings, 'SESSION_COOKIE_NAME', 'ascender_sessionid'))
...
class APIView(views.APIView):
    ...
    def finalize_response(self, request, response, *args, **kwargs):
        ...
        # when you visit a URL that has no matching URL paths in `ascender.api.urls`.
        #
        if response.status_code == 404 and 'ascender.named_url_rewritten' in request.META:
...
def trigger_delayed_deep_copy(*args, **kwargs):
    from ascender.main.tasks.system import deep_copy_model_obj
