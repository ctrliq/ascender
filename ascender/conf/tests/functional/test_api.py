...
from ascender.main.utils.encryption import decrypt_field
...
from ascender.sso import fields as sso_fields
...
def test_setting_singleton_update(api_request, dummy_setting):
    with (
        ...
        mock.patch('ascender.conf.views.clear_setting_cache'),
    ):
...
def test_setting_singleton_update_hybriddictfield_with_forbidden(api_request, dummy_setting):
    ...
    with (
        dummy_setting('FOO_BAR', field_class=sso_fields.SAMLOrgAttrField, category='FooBar', category_slug='foobar'),
        mock.patch('ascender.conf.views.clear_setting_cache'),
    ):
        api_request(
            'patch',
