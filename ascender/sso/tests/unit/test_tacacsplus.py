...
def test_empty_host_fails_auth(tacacsplus_backend):
    with mock.patch('ascender.sso.backends.django_settings') as settings:
...
def test_client_raises_exception(tacacsplus_backend):
    ...
    with (
        mock.patch('ascender.sso.backends.django_settings') as settings,
        mock.patch('ascender.sso.backends.logger') as logger,
        mock.patch('ascender.sso.backends.TACACSClient', return_value=client),
    ):
...
def test_client_return_invalid_fails_auth(tacacsplus_backend):
    ...
    with mock.patch('ascender.sso.backends.django_settings') as settings, mock.patch('ascender.sso.backends.TACACSClient', return_value=client):
        settings.TACACSPLUS_HOST = 'localhost'
        settings.TACACSPLUS_AUTH_PROTOCOL = 'ascii'
        ret_user = tacacsplus_backend.authenticate(None, "user", "pass")
        assert ret_user is None
...
def test_client_return_valid_passes_auth(tacacsplus_backend):
    ...
    with (
        mock.patch('ascender.sso.backends.django_settings') as settings,
        mock.patch('ascender.sso.backends.TACACSClient', return_value=client),
        mock.patch('ascender.sso.backends._get_or_set_enterprise_user', return_value=user),
    ):
        settings.TACACSPLUS_HOST = 'localhost'
        settings.TACACSPLUS_AUTH_PROTOCOL = 'ascii'
        ret_user = tacacsplus_backend.authenticate(None, "user", "pass")
...
def test_backend_binds_the_names_the_installed_tacacs_plus_actually_exports():
    ...
    from ascender.sso import backends
