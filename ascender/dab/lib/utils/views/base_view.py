...
class AnsibleBaseView(APIView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        with ExitStack() as stack:
            from ascender.dab.lib.utils.db import ensure_transaction

            stack.enter_context(ensure_transaction())
            if 'ascender.dab.activitystream' in settings.INSTALLED_APPS:
                from ascender.dab.activitystream import deferred_activity_stream

                stack.enter_context(deferred_activity_stream())
            if 'ascender.dab.rbac' in settings.INSTALLED_APPS:
                from ascender.dab.rbac.triggers import defer_rbac_computations

                stack.enter_context(defer_rbac_computations())
            if 'ascender.dab.resource_registry' in settings.INSTALLED_APPS:
                from ascender.dab.resource_registry.signals.handlers import defer_resource_cleanup
