...
from ascender.dab.lib.utils.models import is_system_user
from ascender.dab.lib.utils.settings import get_setting

oauth2_provider_installed = "ascender.dab.oauth2_provider" in get_setting("INSTALLED_APPS", [])
...
def try_add_oauth2_scope_permission(permission_classes: list):
    ...
    :return: A list of permission_classes, including OAuth2ScopePermission
        if ascender.dab.oauth2_provider is installed; otherwise the same
        permission_classes list supplied to the function
    """
    if oauth2_provider_installed:
        from ascender.dab.oauth2_provider.permissions import OAuth2ScopePermission
