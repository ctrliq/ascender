...
from ascender.main.request_context import get_current_user  # Ascender: in-tree replacement for django-crum
...
from ascender.dab.lib.utils.create_system_user import create_system_user, get_system_username
...
def prevent_search(relation):
    ...
    `ascender.dab.rest_filters.rest_framework.field_lookup_backend.FieldLookupBackend` to block fields and relations that
...
def get_system_user() -> Optional[AbstractUser]:
    from ascender.dab.lib.abstract_models.user import AbstractDABUser
    ...
    if system_username is not None and system_user is None:
        ...
        if 'ascender.dab.resource_registry' in settings.INSTALLED_APPS:
            from ascender.dab.resource_registry.models import ResourceType
