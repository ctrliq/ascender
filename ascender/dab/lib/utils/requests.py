...
def get_remote_hosts(request: HttpRequest, get_first_only: bool = False) -> list[str]:
    ...
    try:
        if 'HTTP_X_TRUSTED_PROXY' in request.META:
            from ascender.dab.jwt_consumer.common.util import validate_x_trusted_proxy_header
...
def is_proxied_request(request: Optional[HttpRequest] = None) -> bool:
    ...
    if request is None:
        from ascender.main.request_context import get_current_request  # Ascender: in-tree replacement for django-crum
