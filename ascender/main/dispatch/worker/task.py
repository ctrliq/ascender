...
from ascender.main.tasks.system import dispatch_startup, inform_cluster_of_shutdown
...
class TaskWorker(BaseWorker):
    ...
    `ascender.main.dispatch.publish`.
    """
    ...
    def resolve_callable(task):
        ...
        ascender.main.tasks.system.delete_inventory
        ascender.main.tasks.jobs.RunProjectUpdate
        """
        # Both prefixes, because a task name travels as a string. A message
        # queued before this rename names awx.main.tasks.system.delete_inventory
        # and is read after it, and the awx alias resolves it to the same
        # callable, so refusing the old prefix would drop everything already in
        # the queue at upgrade time.
        if not task.startswith(TASK_NAME_PREFIXES):
            raise ValueError('{} is not a valid Ascender task'.format(task))
    ...
    def perform_work(self, body):
        ...
        body = {
            ...
            'callbacks': [{
                'args': [],
                'kwargs': {}
                'task': u'ascender.main.tasks.system.handle_work_success'
            }],
            'errbacks': [{
                'args': [],
                'kwargs': {},
                'task': 'ascender.main.tasks.system.handle_work_error'
            }],
            'kwargs': {},
            'task': u'ascender.main.tasks.jobs.RunProjectUpdate'
        }
        """
