...
from ascender.dab.lib.utils.models import prevent_search

# Ascender
from ascender.api.versioning import reverse
from ascender.main.constants import CLOUD_PROVIDERS
from ascender.main.consumers import emit_channel_notification
from ascender.main.fields import (
    ImplicitRoleField,
    SmartFilterField,
    OrderedManyToManyField,
)
from ascender.main.managers import HostManager, HostMetricActiveManager
from ascender.main.models.base import BaseModel, CommonModelNameNotUnique, VarsDictProperty, CLOUD_INVENTORY_SOURCES, accepts_json
from ascender.main.models.events import InventoryUpdateEvent, UnpartitionedInventoryUpdateEvent
from ascender.main.models.unified_jobs import UnifiedJob, UnifiedJobTemplate
from ascender.main.models.mixins import (
    ResourceMixin,
    TaskManagerInventoryUpdateMixin,
    RelatedJobsMixin,
)
from ascender.main.models.notifications import (
    NotificationTemplate,
    JobNotificationMixin,
)
from ascender.main.models.credential.injectors import _openstack_data
...
class Inventory(CommonModelNameNotUnique, ResourceMixin, RelatedJobsMixin):
    ...
    def get_absolute_url(self, request=None):
        if request is not None:
            # circular import
            from ascender.api.urls.inventory import constructed_inventory_urls, federated_inventory_urls
    ...
    def update_computed_fields(self):
        ...
        from ascender.main.models.jobs import JobHostSummary  # circular import: inventory.py loads before jobs.py
    ...
    def schedule_deletion(self, user_id=None):
        from ascender.main.tasks.system import delete_inventory
        from ascender.main.signals import activity_stream_delete
...
class Group(CommonModelNameNotUnique, RelatedJobsMixin):
    ...
    def delete_recursive(self):
        ...
        from ascender.main.tasks.system import update_inventory_computed_fields
        from ascender.main.signals import disable_activity_stream, activity_stream_delete
    ...
    @property
    def job_events(self):
        from ascender.main.models.jobs import JobEvent
...
class HostMetric(models.Model):
    ...
    deleted = models.BooleanField(default=False, help_text=_('Boolean flag saying whether the host is deleted and therefore no longer counted as automated'))
...
class HostMetricSummaryMonthly(models.Model):
    """
    HostMetric summaries computed by scheduled task 'ascender.main.tasks.system.host_metric_summary_monthly' monthly
    """

    date = models.DateField(unique=True)
    hosts_added = models.IntegerField(default=0, help_text=_("How many hosts were first automated in the associated month"))
    hosts_deleted = models.IntegerField(default=0, help_text=_("How many hosts were deleted in the associated month"))
...
class InventoryUpdate(UnifiedJob, InventorySourceOptions, JobNotificationMixin, TaskManagerInventoryUpdateMixin):
    ...
    def _get_task_class(cls):
        from ascender.main.tasks.jobs import RunInventoryUpdate
...
class PluginFileInjector(object):
    ...
    def _get_shared_env(self, inventory_update, private_data_dir, private_data_files):
        ...
        if self.base_injector == 'managed':
            from ascender.main.models.credential import injectors as builtin_injectors
