...
def is_testing(argv=None):
    ...
    if (os.environ.get('DJANGO_SETTINGS_MODULE') or '').startswith('ascender.main.tests.settings_for_test'):
        return True
...
def get_ascender_version():
    ...
    """
    from ascender import __version__
...
class ScheduleTaskManager(ScheduleManager):
    def __init__(self):
        from ascender.main.scheduler.tasks import task_manager
...
class ScheduleDependencyManager(ScheduleManager):
    def __init__(self):
        from ascender.main.scheduler.tasks import dependency_manager
...
class ScheduleWorkflowManager(ScheduleManager):
    def __init__(self):
        from ascender.main.scheduler.tasks import workflow_manager
...
def truncate_stdout(stdout, size):
    from ascender.main.constants import ANSI_SGR_PATTERN
...
def cleanup_new_process(func):
    ...
    def wrapper_cleanup_new_process(*args, **kwargs):
        from ascender.conf.settings import SettingsWrapper  # noqa
