...
from ascender.main import migrations, request_context
from ascender.main.request_context import get_current_request, get_current_user, impersonate  # noqa: F401 -- long-standing import location
...
class ThreadLocalMiddleware:
    """
    Binds the current request into ascender.main.request_context for access
    throughout the request lifecycle, restoring the previous state (and
    discarding leaked impersonation) once the request completes, even if an
    exception occurs. Despite the historical name it is contextvars-based,
    so it works under both WSGI and ASGI.
    """
    ...
    def __call__(self, request):
        ...
        token = request_context.set_current_request(request)
        try:
            return self.get_response(request)
        finally:
            request_context.end_request_context(token)
...
def _customize_graph():
    from ascender.main.models import Instance, Schedule, UnifiedJobTemplate
...
class URLModificationMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        if request.path_info != new_path:
            request.META['ascender.named_url_rewritten'] = request.path
