...
from ascender.main.dispatch.periodic import Scheduler
...
def _disable_database_settings(mocker):
    m = mocker.patch('ascender.conf.settings.SettingsWrapper.all_supported_settings', new_callable=mock.PropertyMock)
    m.return_value = []
...
class TestTaskDispatcher:
    ...
    def test_function_dispatch(self):
        result = self.tm.perform_work({'task': 'ascender.main.tests.functional.test_dispatch.add', 'args': [2, 2]})
        assert result == 4

    def test_function_dispatch_must_be_decorated(self):
        result = self.tm.perform_work({'task': 'ascender.main.tests.functional.test_dispatch.restricted', 'args': [2, 2]})
        assert isinstance(result, ValueError)
        assert str(result) == 'ascender.main.tests.functional.test_dispatch.restricted is not decorated with @task()'  # noqa

    def test_method_dispatch(self):
        result = self.tm.perform_work({'task': 'ascender.main.tests.functional.test_dispatch.Adder', 'args': [2, 2]})
        assert result == 4
    ...
    def test_python_function_cannot_be_imported(self):
        ...
        assert isinstance(result, ValueError)
        assert str(result) == 'os.system is not a valid Ascender task'  # noqa

    def test_undefined_function_cannot_be_imported(self):
        result = self.tm.perform_work({'task': 'ascender.foo.bar'})
        assert isinstance(result, ModuleNotFoundError)
        assert str(result) == "No module named 'ascender.foo'"  # noqa
...
class TestTaskPublisher:
    ...
    def test_function_apply_async(self):
        ...
        assert message['task'] == 'ascender.main.tests.functional.test_dispatch.add'
        assert queue == 'foobar'
    ...
    def test_method_apply_async(self):
        ...
        assert message['args'] == [2, 2]
        assert message['kwargs'] == {}
        assert message['task'] == 'ascender.main.tests.functional.test_dispatch.Adder'
        assert queue == 'foobar'

    def test_apply_async_queue_required(self):
        with pytest.raises(ValueError) as e:
            message, queue = add.apply_async([2, 2])
        assert "ascender.main.tests.functional.test_dispatch.add: Queue value required and may not be None" == e.value.args[0]
...
class TestScheduler:
    ...
    def test_missed_schedule(self, mocker):
        ...
        mocker.patch('ascender.main.dispatch.periodic.time.time', return_value=scheduler.global_start + 50)
    ...
    def test_advance_schedule(self, mocker):
        ...
        for job in scheduler.jobs:
            ...
            job.offset = 0.0
        mocker.patch('ascender.main.dispatch.periodic.time.time', return_value=scheduler.global_start + 29)
        to_run = scheduler.get_and_mark_pending()
        assert set(job.name for job in to_run) == set(['joba', 'jobb'])
        mocker.patch('ascender.main.dispatch.periodic.time.time', return_value=scheduler.global_start + 39)
        to_run = scheduler.get_and_mark_pending()
        assert len(to_run) == 1
    ...
    def test_scheduler_debug(self, mocker):
        ...
        mocker.patch('ascender.main.dispatch.periodic.time.time', return_value=current_time - 1.0e-8)
...
@pytest.mark.django_db
class TestTaskNamePrefixes:
    """A task name travels as a string, so both prefixes have to resolve.

    A message queued before the package rename names
    `awx.main.tasks.system.delete_inventory` and is read by a worker running
    after it. Refusing the old prefix would drop everything already in the queue
    at upgrade time, which is the one failure the awx alias exists to prevent.
    """

    def test_the_old_prefix_still_resolves(self):
        assert TaskWorker.resolve_callable('awx.main.tests.functional.test_dispatch.add') is add

    def test_the_new_prefix_resolves(self):
        assert TaskWorker.resolve_callable('ascender.main.tests.functional.test_dispatch.add') is add

    def test_both_names_reach_the_same_callable(self):
        assert TaskWorker.resolve_callable('awx.main.tests.functional.test_dispatch.add') is TaskWorker.resolve_callable(
            'ascender.main.tests.functional.test_dispatch.add'
        )
