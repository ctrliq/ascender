...
# Ascender models
from ascender.main.models import ActivityStream, Organization, JobTemplate, Credential, CredentialType, Inventory, InventorySource, Label, Project, User

# other Ascender
from ascender.main.utils import model_to_dict, model_instance_diff
from ascender.main.utils.common import get_allowed_fields
from ascender.main.signals import model_serializer_mapping
...
def test_anon_user_action():
    with mock.patch('ascender.main.middleware.get_current_user') as u_mock:
...
def test_cluster_node_recorded(inventory, project):
    ...
    with mock.patch('ascender.main.models.activity_stream.settings.CLUSTER_HOST_ID', 'foo_host'):
...
def test_cluster_node_long_node_name(inventory, project):
    jt = JobTemplate.objects.create(name='testjt', inventory=inventory, project=project)
    with mock.patch('ascender.main.models.activity_stream.settings.CLUSTER_HOST_ID', 'f' * 700):
        job = jt.create_unified_job()
...
def test_credential_defaults_idempotency():
    ...
    old_inputs = CredentialType.objects.get(name='CIQ Ascender Automation Platform', kind='cloud').inputs
...
def test_associate_emits_every_entry(organization, job_template, django_capture_on_commit_callbacks):
    ...
    with mock.patch('ascender.main.signals.emit_activity_stream_change') as emit:
