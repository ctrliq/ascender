...
from ascender.main.models import Credential, UnifiedJob, Instance
from ascender.main.tests.factories import (
    create_organization,
    create_job_template,
    create_instance,
...
from ascender.conf.models import Setting as ConfSetting
from ascender.conf.signals import on_post_save_setting
...
def mock_access():
    ...
    def access_given_class(TowerClass):
        try:
            ...
            the_patch = mock.patch.dict('ascender.main.access.access_registry', {TowerClass: MockAccess}, clear=False)
            the_patch.__enter__()
            yield mock_instance
        finally:
            the_patch.__exit__()
...
def pytest_runtest_teardown(item, nextitem):
    ...
    for entry in post_save.receivers:
        ...
        if receiver is on_post_save_setting or (isinstance(receiver, weakref.ReferenceType) and receiver() is on_post_save_setting):
            connected = True
            break
    assert connected, '{} left ascender.conf.signals.on_post_save_setting disconnected from post_save'.format(item.nodeid)
...
def mock_get_event_queryset_no_job_created():
    """
    Do not add the faked job_created field to the filter. If we do, it will result in an sql query for the
