...
ASCENDER_LOGGING_MODE decides it: 'file' writes to /var/log/ascender, which is what a
...
def build_logging(mode):
    ...
    try:
        module = importlib.import_module('ascender.settings.defaults')
        ...
        importlib.reload(sys.modules['ascender.settings.defaults'])
