...
from ascender.main.tasks.system import awx_periodic_scheduler, inspect_execution_and_hop_nodes, update_inventory_computed_fields
...
@pytest.fixture
def mock_logger():
    with patch("ascender.main.tasks.system.logger") as logger:
        yield logger
...
def test_update_inventory_computed_fields_existing_inventory(mock_logger, mock_inventory):
    ...
    with patch("ascender.main.tasks.system.Inventory.objects.filter") as mock_filter:
...
class TestInspectExecutionAndHopNodes:
    ...
    @patch("ascender.main.tasks.system.inspect_established_receptor_connections")
    @patch("ascender.main.tasks.system.get_receptor_ctl")
    @patch("ascender.main.tasks.system.advisory_lock")
