...
# Ascender
from ascender.api.generics import ParentMixin, SubListCreateAttachDetachAPIView, SubListAttachDetachAPIView, ResourceAccessList, ListAPIView
from ascender.main.models import Organization, Credential


@pytest.fixture
def get_object_or_404(mocker):
    # pytest patch without return_value generates a random value, we are counting on this
    return mocker.patch('ascender.api.generics.get_object_or_404')


@pytest.fixture
def get_object_or_400(mocker):
    return mocker.patch('ascender.api.generics.get_object_or_400')
...
def parent_relationship_factory(mocker):
    def rf(serializer_class, relationship_name, relationship_value=mocker.Mock()):
        ...
        mocker.patch('ascender.api.generics.ParentMixin.get_parent_object', return_value=mock_parent_relationship)
...
class TestResourceAccessList:
    ...
    def test_parent_access_check_failed(self, mocker, mock_organization):
        ...
        mocker.patch('ascender.main.access.BaseAccess.can_read', mock_access)
