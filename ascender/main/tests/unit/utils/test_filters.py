...
# Ascender
from ascender.main.utils.filters import SmartFilter, ExternalLoggerEnabled
...
@mock.patch('ascender.main.utils.filters.get_model', return_value=mockHost())
class TestSmartFilterQueryFromString:
    @mock.patch(
        'ascender.dab.rest_filters.rest_framework.field_lookup_backend.get_fields_from_path', lambda model, path, **kwargs: ([model], path)
