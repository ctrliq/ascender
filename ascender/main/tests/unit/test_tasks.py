...
from ascender.main.models.credential import HIDDEN_PASSWORD, ManagedCredentialType

from ascender.main.tasks import jobs, system, receptor
from ascender.main.utils import encrypt_field, encrypt_value
...
def mock_create_partition():
    with mock.patch('ascender.main.tasks.jobs.create_partition') as cp_mock:
        yield cp_mock
...
def test_send_notifications_job_id(mocker):
    mocker.patch('ascender.main.models.UnifiedJob.objects.get')
    system.send_notifications([], job_id=1)
    assert UnifiedJob.objects.get.called
    UnifiedJob.objects.get.assert_called_with(id=1)


@mock.patch('ascender.main.models.UnifiedJob.objects.get')
@mock.patch('ascender.main.models.Notification.objects.filter')
...
class TestGenericRun:
    def test_generic_failure(self, patch_Job, execution_environment, mock_me, mock_create_partition, mocker):
        ...
        with mock.patch('ascender.main.tasks.jobs.shutil.copytree'):
    ...
    def test_awx_task_env(self, patch_Job, private_data_dir, execution_environment, mock_me):
        ...
        with mock.patch('ascender.main.tasks.jobs.settings.ASCENDER_TASK_ENV', {'FOO': 'BAR'}):
...
@mock.patch('os.open')
@mock.patch('ascender.main.tasks.jobs.logger')
def test_acquire_lock_open_fail_logged(logger, os_open, mock_me):
    err = OSError()
    err.errno = 3
    ...
    logger.error.assert_called_with("I/O error({0}) while trying to open lock file [{1}]: {2}".format(3, 'this_file_does_not_exist', 'dummy message'))
...
def test_acquire_lock_acquisition_fail_logged(fcntl_lockf, logger, os_close, os_open, mock_me):
    err = IOError()
    err.errno = 3
    err.strerror = 'dummy message'
...
@mock.patch('os.close')
@mock.patch('fcntl.lockf')
@mock.patch('ascender.main.tasks.jobs.signal_callback', return_value=True)
...
@mock.patch('ascender.main.tasks.jobs.BaseTask.release_lock')
@mock.patch('ascender.main.tasks.jobs.BaseTask.acquire_lock', return_value=True)
@mock.patch('ascender.main.tasks.jobs.SourceControlMixin.sync_and_copy_without_lock')
@mock.patch('ascender.main.tasks.jobs.SourceControlMixin.get_sync_needs')
...
@mock.patch('ascender.main.tasks.jobs.BaseTask.release_lock')
@mock.patch('ascender.main.tasks.jobs.BaseTask.acquire_lock')
@mock.patch('ascender.main.tasks.jobs.SourceControlMixin.sync_and_copy_without_lock')
@mock.patch('ascender.main.tasks.jobs.SourceControlMixin.get_sync_needs')
...
@mock.patch('ascender.main.tasks.jobs.BaseTask.release_lock')
@mock.patch('ascender.main.tasks.jobs.SourceControlMixin.sync_and_copy_without_lock')
@mock.patch('ascender.main.tasks.jobs.SourceControlMixin.get_sync_needs', return_value=['update_git'])
def test_sync_and_copy_canceled_during_lock_upgrade(get_sync_needs, sync_without_lock, release_lock, mock_me):
    ...
    with mock.patch('ascender.main.tasks.jobs.BaseTask.acquire_lock', side_effect=[True, False]):
...
@mock.patch('ascender.main.tasks.jobs.BaseTask.release_lock')
@mock.patch('ascender.main.tasks.jobs.SourceControlMixin.sync_and_copy_without_lock')
@mock.patch('ascender.main.tasks.jobs.SourceControlMixin.get_sync_needs', side_effect=[['update_git'], []])
def test_sync_and_copy_downgrades_lock_when_sync_done_elsewhere(get_sync_needs, sync_without_lock, release_lock, fcntl_lockf, mock_me):
    ...
    with mock.patch('ascender.main.tasks.jobs.BaseTask.acquire_lock', side_effect=fake_acquire):
        task.sync_and_copy(project, '/fake/private_data_dir')
...
@mock.patch('ascender.main.tasks.jobs.BaseTask.release_lock')
@mock.patch('ascender.main.tasks.jobs.SourceControlMixin.sync_and_copy_without_lock')
@mock.patch('ascender.main.tasks.jobs.SourceControlMixin.get_sync_needs', side_effect=[['update_git'], ['update_git']])
...
def test_managed_injector_redaction(injector_cls):
    """See ascender.main.models.inventory.PluginFileInjector._get_shared_env
    The ordering within ascender.main.tasks.jobs.BaseTask and contract with build_env
