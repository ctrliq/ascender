...
from ascender.main.middleware import SettingsCacheMiddleware
...
def test_the_first_request_drops_the_cache(middleware, memoized):
    with mock.patch('ascender.main.middleware.cache', read_returns(7)):
...
def test_an_unchanged_version_leaves_the_cache_alone(middleware, memoized):
    with mock.patch('ascender.main.middleware.cache', read_returns(7, 7, 7)):
        for _ in range(3):
            middleware.process_request(mock.Mock())
...
def test_a_moved_version_drops_the_cache_once(middleware, memoized):
    with mock.patch('ascender.main.middleware.cache', read_returns(7, 8, 8)):
        for _ in range(3):
            middleware.process_request(mock.Mock())
...
def test_it_reads_one_key(middleware):
    ...
    with mock.patch('ascender.main.middleware.cache', cache):
        middleware.process_request(mock.Mock())
...
def test_a_missing_version_is_still_a_version(middleware, memoized):
    ...
    with mock.patch('ascender.main.middleware.cache', read_returns(None, None)):
        middleware.process_request(mock.Mock())
        middleware.process_request(mock.Mock())
