...
import ascender.main.notifications.irc_backend as irc_backend
...
def backend_with(sock, **kwargs):
    ...
    with mock.patch('ascender.main.notifications.irc_backend.socket.create_connection', return_value=sock):
...
def test_connection_failure_is_raised():
    ...
    with mock.patch('ascender.main.notifications.irc_backend.socket.create_connection', side_effect=OSError('refused')):
        try:
            backend.open()
        except OSError as e:
