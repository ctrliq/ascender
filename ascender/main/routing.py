...
from ascender.dab.lib.channels.middleware import DrfAuthMiddlewareStack
...
def __getattr__(name: str) -> ProtocolTypeRouter:
    ...
    Normally, someone would get application from this module via:
        from ascender.main.routing import application
    ...
    ...
        ascender.main.routing.__getattribute__(...)                              # <-- we don't define this so NOOP as far as we are concerned
        if '__getattr__' in ascender.main.routing.__dict__:                      # <-- this triggers the function we are in
            return ascender.main.routing.__dict__.__getattr__("application")
