...
class ImplicitRoleField(models.ForeignKey):
    ...
    def bind_m2m_changed(self, _self, _role_class, cls):
        ...
        for field_name in field_names:
            ...
            # consistency is assured by unit test ascender.main.tests.functional
    ...
    def _post_save(self, instance, created, *args, **kwargs):
        ...
        # Avoid circular import
        from ascender.main.models.rbac import batch_role_ancestor_rebuilding, Role
...
class DynamicCredentialInputField(JSONSchemaField):
    ...
    `ascender.main.models.credential:CredentialInputSource().metadata`.
...
class CredentialInputField(JSONSchemaField):
    """
    Used to validate JSON for
    `ascender.main.models.credential:Credential().inputs`.
...
class CredentialTypeInputField(JSONSchemaField):
    """
    Used to validate JSON for
    `ascender.main.models.credential:CredentialType().inputs`.
    """
...
class CredentialTypeInjectorField(JSONSchemaField):
    """
    Used to validate JSON for
    `ascender.main.models.credential:CredentialType().injectors`.
    """
