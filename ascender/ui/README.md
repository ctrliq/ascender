...
```shell
TARGET='https://awx.local:8043' npm --prefix ascender/ui start
```

## Testing
```shell
# run code formatting check
npm --prefix ascender/ui run prettier-check

# run lint checks
npm --prefix ascender/ui run lint

# run all unit tests
npm --prefix ascender/ui run test

# run a single test (in this case the login page test):
npm --prefix ascender/ui test -- src/screens/Login/Login.test.tsx

# start the test watcher and run tests on files that you've changed
npm --prefix ascender/ui run test-watch

# start the tests and get the coverage report after the tests have completed
npm --prefix ascender/ui run test -- --coverage
```
#### Note:
...
(tools_ascender_1)$ make clean-ui && npm --prefix=ascender/ui ci

# add an exact development dependency
(tools_ascender_1)$ npm --prefix ascender/ui install --save-dev --save-exact dev-package@1.2.3

# add an exact production dependency
(tools_ascender_1)$ npm --prefix ascender/ui install --save --save-exact prod-package@1.23

# remove a development dependency
(tools_ascender_1)$ npm --prefix ascender/ui uninstall --save-dev dev-package

# remove a production dependency
(tools_ascender_1)$ npm --prefix ascender/ui uninstall --save prod-package

# exit the container
(tools_ascender_1)$ exit

# add the updated package.json and package-lock.json files to scm
git add ascender/ui/package.json ascender/ui/package-lock.json
```
#### Note:
...
```shell
# built files are placed in ascender/ui/build
npm --prefix ascender/ui run build
```
