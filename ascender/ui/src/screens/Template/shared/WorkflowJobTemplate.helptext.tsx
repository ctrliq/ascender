...
// Keyed by what the form, the webhook subform and the detail screen ask for.
// They have read camelCase names since the initial import while this returned
// the API's own field names, so none of these ever reached a popover.
function getHelpText() {
  return {
    inventory: t`Select the inventory containing the hosts you want this workflow to manage.`,
    limit: t`Provide a host pattern to further constrain the list of hosts that will be managed or affected by the workflow. Multiple patterns are allowed. Refer to Ansible documentation for more information and examples on patterns.`,
    ...
    labels: t`Optional labels that describe this workflow job template, such as 'dev' or 'test'. Labels can be used to group and filter workflow job templates and completed jobs.`,
