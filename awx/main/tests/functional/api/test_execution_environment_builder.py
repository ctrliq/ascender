...
# ---------------------------------------------------------------------------
# Project + execution environment file
# ---------------------------------------------------------------------------


VALID_EE_CONTENT = 'version: 3\nimages:\n  base_image:\n    name: quay.io/fedora:latest\n'


def _write_ee_file(directory, relpath, content):
...
@pytest.mark.django_db
class TestExecutionEnvironmentBuilderProjectFile:
    def test_project_execution_environment_files_endpoint(self, get, admin, project, tmp_path):
        _write_ee_file(str(tmp_path), 'execution-environment.yml', VALID_EE_CONTENT)
        _write_ee_file(str(tmp_path), 'nested/execution-environment.yaml', VALID_EE_CONTENT)
        _write_ee_file(str(tmp_path), 'unrelated.yml', 'foo: bar\n')
        url = reverse('api:project_execution_environment_files', kwargs={'pk': project.pk})
        with mock.patch.object(Project, 'get_project_path', return_value=str(tmp_path)):
            r = get(url, admin, expect=200)
        assert sorted(r.data) == ['execution-environment.yml', 'nested/execution-environment.yaml']

    def test_create_with_valid_version_3_file(self, post, admin, organization, project, tmp_path):
        _write_ee_file(str(tmp_path), 'execution-environment.yml', VALID_EE_CONTENT)
        url = reverse('api:execution_environment_builder_list')
        with mock.patch.object(Project, 'get_project_path', return_value=str(tmp_path)):
            r = post(
        ...
        assert r.data['project'] == project.pk
        assert r.data['execution_environment_file'] == 'execution-environment.yml'

    def test_create_rejects_file_without_version_3(self, post, admin, organization, project, tmp_path):
        _write_ee_file(str(tmp_path), 'execution-environment.yml', 'version: 2\nimages: {}\n')
        url = reverse('api:execution_environment_builder_list')
        with mock.patch.object(Project, 'get_project_path', return_value=str(tmp_path)):
            r = post(
                url,
                {
                    'name': 'bad-version-builder',
                    'organization': organization.pk,
                    'project': project.pk,
                    'execution_environment_file': 'execution-environment.yml',
                },
        ...
        assert 'version' in str(r.data['execution_environment_file']).lower()

    def test_create_rejects_missing_file(self, post, admin, organization, project, tmp_path):
        url = reverse('api:execution_environment_builder_list')
        with mock.patch.object(Project, 'get_project_path', return_value=str(tmp_path)):
            r = post(
                url,
                {
                    'name': 'missing-file-builder',
                    'organization': organization.pk,
                    'project': project.pk,
                    'execution_environment_file': 'does-not-exist.yml',
                },
                admin,
                expect=400,
            )
    ...
    def test_execution_environment_file_requires_project(self, post, admin, organization):
        url = reverse('api:execution_environment_builder_list')
        r = post(
            url,
            {
                'name': 'no-project-builder',
                'organization': organization.pk,
                'execution_environment_file': 'execution-environment.yml',
            },
            admin,
    ...
    def test_cannot_use_project_without_use_role(self, post, ee_admin, organization, project):
        """An EE admin without use_role on the project cannot assign it."""
        url = reverse('api:execution_environment_builder_list')
        post(
            url,
            {'name': 'unauthorized-project-builder', 'organization': organization.pk, 'project': project.pk},
            ee_admin,
            expect=403,
        )

    def test_ee_admin_can_use_project_with_use_role(self, post, ee_admin, organization, project):
        """Granting use_role on the project lets an EE admin assign it."""
        project.use_role.members.add(ee_admin)
        url = reverse('api:execution_environment_builder_list')
        r = post(
            url,
            {'name': 'authorized-project-builder', 'organization': organization.pk, 'project': project.pk},
            ee_admin,
            expect=201,
        )
        assert r.data['project'] == project.pk
