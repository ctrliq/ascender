...
@pytest.mark.parametrize(
    ...
    [
        ('awx.main.models.events', 'analytics_logger', 'job_events'),
        ('awx.main.signals', 'analytics_logger', 'activity_stream'),
        ('awx.main.models.unified_jobs', 'logger_job_lifecycle', 'job_lifecycle'),
        ('awx.main.tasks.facts', 'system_tracking_logger', 'system_tracking'),
        ('awx.main.analytics.broadcast_websocket', 'logger', 'broadcast_websocket'),
        ('awx.main.middleware', 'perf_logger', 'performance'),
        ('awx.api.generics', 'analytics_logger', 'performance'),
    ],
)
...
def test_the_suffix_is_what_the_aggregator_setting_matches_on():
    ...
    """
    from awx.main.utils.filters import ExternalLoggerEnabled
