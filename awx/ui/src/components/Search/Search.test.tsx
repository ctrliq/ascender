...
describe('<Search />', () => {
  ...
  test('filter keys are properly labeled', () => {
    ...
    expect(
      within(typeGroup as unknown as HTMLElement).getByText('Foo Bar!')
    ).toBeInTheDocument();
    ...
    const nameGroup = screen
      ...
      .closest('.pf-v6-c-label-group');
    expect(
      within(nameGroup as unknown as HTMLElement).getByText('bar')
    ).toBeInTheDocument();
  });
  ...
  test('should test handle remove of option-based with empty string value', async () => {
    ...
    await user.click(within(chips[0] as HTMLElement).getByRole('button'));
  ...
  test("ToolbarFilter added for any key that doesn't have search column", () => {
    ...
    const nameExactGroup = screen
      ...
      .closest('.pf-v6-c-label-group');
    expect(
      within(nameExactGroup as unknown as HTMLElement).getByText('baz')
    ).toBeInTheDocument();

    const fooGroup = screen.getByText('foo').closest('.pf-v6-c-label-group');
    expect(
      within(fooGroup as unknown as HTMLElement).getByText('bar')
    ).toBeInTheDocument();
  });
