...
function AdvancedSearch({
  ...
  const { t } = useLingui();
  const prefixOptions = (enableNegative: unknown) => {
    const opts = [
      {
  ...
  const [prefixSelection, setPrefixSelection] = useState<string | null>(null);
  const [lookupSelection, setLookupSelection] = useState<string | null>(null);
  const [keySelection, setKeySelection] = useState<string | null>(null);
  ...
  const handleAdvancedSearch = (e: React.SyntheticEvent) => {
  ...
  const handleAdvancedTextKeyDown = (e: React.KeyboardEvent) => {
