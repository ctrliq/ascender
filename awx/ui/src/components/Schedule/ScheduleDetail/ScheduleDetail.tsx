...
function ScheduleDetail({
  ...
  let frequencyOptions: FrequencyOptionsMap = {};
  let exceptionFrequency: ScheduleFrequency[] = [];
  let exceptionOptions: FrequencyOptionsMap = {};
  try {
  ...
  if (hasDaysToKeepField && extra_data) {
    if (typeof extra_data === 'string' && extra_data !== '') {
      daysToKeep = parseVariableField(extra_data).days as number;
    }
    if (typeof extra_data === 'object') {
      daysToKeep = (extra_data as { days?: number })?.days ?? null;
    }
  }
