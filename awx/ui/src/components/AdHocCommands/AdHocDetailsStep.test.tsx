...
describe('<AdHocDetailsStep />', () => {
  ...
  test('should update form values', async () => {
    ...
    await user.selectOptions(moduleSelect as unknown as Element, 'command');
    await user.clear(argsInput as unknown as Element);
    await user.type(argsInput as unknown as Element, 'foo');
    await user.clear(limitInput as unknown as Element);
    await user.type(
      limitInput as unknown as Element,
      'Inventory 1, inventory 2, new inventory'
    );
    // select verbosity by its stable option value ('1'), not the i18n label
    await user.selectOptions(verbositySelect as unknown as Element, '1');
    await user.clear(forksInput as unknown as Element);
    await user.type(forksInput as unknown as Element, '10');
