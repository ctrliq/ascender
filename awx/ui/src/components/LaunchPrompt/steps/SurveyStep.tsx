...
function SurveyStep({ surveyConfig }: SurveyStepProps) {
  const fieldTypes = {
    text: TextField,
    textarea: TextField,
    password: TextField,
  ...
  return (
    <div data-cy="survey-prompts">
      ...
      >
        {surveyConfig.spec?.map((question: SurveyQuestion) => {
          const Field = fieldTypes[question.type as keyof typeof fieldTypes];
...
function TextField({ question }: { question: SurveyQuestion }) {
  const validators = [
...
function NumberField({ question }: { question: SurveyQuestion }) {
  const validators = [
    question.required ? required(null) : null,
    minMaxValue(question.min as number, question.max as number),
...
function MultipleChoiceField({ question }: { question: SurveyQuestion }) {
  const { t } = useLingui();
  const [field, meta, helpers] = useField({
    name: `survey_${question.variable}`,
    validate: question.required ? required(null) : undefined,
  });
...
function MultiSelectField({ question }: { question: SurveyQuestion }) {
  const { t } = useLingui();
  const [isOpen, setIsOpen] = useState(false);
  const [filterValue, setFilterValue] = useState('');
  const [field, meta, helpers] = useField({
      ...
      <Select
        ...
        onSelect={(_event, value) => {
          if (field?.value?.includes(value)) {
            helpers.setValue(field.value.filter((o: unknown) => o !== value));
          } else {
                  ...
                  <LabelGroup>
                    {field.value.map((val: string) => (
                      <Label
                        ...
                        onClose={() => {
                          helpers.setValue(
                            field.value.filter((o: string) => o !== val)
                          );
                          helpers.setTouched(true);
                        }}
                      >
