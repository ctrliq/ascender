...
function useStorage(
  ...
  const setValue = (val: unknown) => {
    ...
    setStorageVal(val as string | null);
  };
...
function SessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const sessionTimeoutId = useRef<ReturnType<typeof setTimeout> | undefined>(
    undefined
  );
  const sessionIntervalId = useRef<ReturnType<typeof setInterval> | undefined>(
    undefined
  );
  ...
  const {
    ...
    useCallback(async () => {
      const { data } = await RootAPI.read<{
        login_redirect_override?: string | null;
      }>();
      return {
        loginRedirectOverride: data?.login_redirect_override ?? null,
      };
    }, []),
    {
  ...
  const loginRedirectOverride = loginRedirectResult?.loginRedirectOverride;
  ...
  const logout = useCallback(async () => {
    ...
    if (!isSessionExpired.current) {
      ...
      // The stored id is cleared on logout. Removing it says that more
      // plainly than writing the string "null" into it, which is what
      // passing null here used to do.
