...
describe('<JobTemplateDetail />', () => {
  ...
  test('should render instance groups link', async () => {
    ...
    expect(
      within(instanceGroups as unknown as HTMLElement).getByRole('link', {
        name: 'IG2',
      })
  ...
  test('should show content error for failed instance group fetch', async () => {
    vi.mocked(JobTemplatesAPI.readInstanceGroups).mockImplementationOnce(() =>
  ...
  test('Error dialog shown for failed deletion', async () => {
    vi.mocked(JobTemplatesAPI.destroy).mockImplementationOnce(() =>
      Promise.reject(new Error())
    );
    const { user } = await renderDefault();
    await user.click(screen.getByRole('button', { name: 'Delete' }));
