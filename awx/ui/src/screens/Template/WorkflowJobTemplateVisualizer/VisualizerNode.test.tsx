...
vi.mocked(WorkflowJobTemplateNodesAPI.readCredentials).mockResolvedValue({
  data: {
    results: [],
  },
...
describe('VisualizerNode', () => {
        ...
        <WorkflowDispatchContext.Provider value={dispatch}>
          <WorkflowStateContext.Provider
            value={{
              ...mockedContext,
              addingLink: true,
              addLinkSourceNode: 323 as unknown as WorkflowNode,
            }}
          >
            <svg>
              <VisualizerNode
  ...
  describe('Node without full unified job template', () => {
    ...
    test('Displays error fetching full unified job template', async () => {
      vi.mocked(JobTemplatesAPI.readDetail).mockRejectedValueOnce(
    ...
    test('Attempts to fetch credentials on view', async () => {
      vi.mocked(JobTemplatesAPI.readDetail).mockResolvedValueOnce({
        data: {
          id: 7,
          name: 'Example',
        },
