...
describe('<WorkflowJobTemplateDetail/>', () => {
  ...
  test('expect detail fields to render properly', () => {
    ...
    expect(
      within(labels as unknown as HTMLElement).getByText('Label 1')
    ).toBeInTheDocument();
    expect(
      within(labels as unknown as HTMLElement).getByText('Label 2')
    ).toBeInTheDocument();
    expect(
      within(labels as unknown as HTMLElement).getByText('Label 3')
    ).toBeInTheDocument();
  ...
  test('link out resource have the correct url', () => {
    ...
    const inventory = within(
      getDetailValue('Inventory') as HTMLElement
    ).getByRole('link');
    const organization = within(
      getDetailValue('Organization') as HTMLElement
    ).getByRole('link');
