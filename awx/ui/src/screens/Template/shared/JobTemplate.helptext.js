...
function getHelpText(i18n) {
  return {
    jobType: i18n._(
      t`For job templates, select run to execute the playbook. Select check to only check playbook syntax, test environment setup, and report problems without executing the playbook.`
    ),
    inventory: i18n._(
      t`Select the inventory containing the hosts you want this job to manage.`
    ),
    project: i18n._(
      t`Select the project containing the playbook you want this job to execute.`
    ),
    executionEnvironmentForm: i18n._(
      t`The container image to be used for execution.`
    ),
    executionEnvironmentDetail: i18n._(
      t`The execution environment that will be used when launching this job template. The resolved execution environment can be overridden by explicitly assigning a different one to this job template.`
    ),
    playbook: i18n._(t`Select the playbook to be executed by this job.`),
    credentials: i18n._(
      t`Select credentials for accessing the nodes this job will be ran against. You can only select one credential of each type. For machine credentials (SSH), checking "Prompt on launch" without selecting credentials will require you to select a machine credential at run time. If you select credentials and check "Prompt on launch", the selected credential(s) become the defaults that can be updated at run time.`
    ),
    labels: i18n._(
    ...
    limit: i18n._(
      t`Provide a host pattern to further constrain the list of hosts that will be managed or affected by the playbook. Multiple patterns are allowed. Refer to Ansible documentation for more information and examples on patterns.`
    ),
    verbosity: i18n._(
      t`Control the level of output ansible will produce as the playbook executes.`
    ),
    jobSlicing: i18n._(
      t`Divide the work done by this job template into the specified number of job slices, each running the same tasks against a portion of the inventory.`
    ),
    timeout: i18n._(
      t`The amount of time (in seconds) to run before the job is canceled. Defaults to 0 for no job timeout.`
    ),
    showChanges: i18n._(
    ...
    instanceGroups: i18n._(
      t`Select the Instance Groups for this Job Template to run on.`
    ),
    jobTags: i18n._(
      t`Tags are useful when you have a large playbook, and you want to run a specific part of a play or task. Use commas to separate multiple tags. Refer to the documentation for details on the usage of tags.`
    ),
    skipTags: i18n._(
      t`Skip tags are useful when you have a large playbook, and you want to skip specific parts of a play or task. Use commas to separate multiple tags. Refer to the documentation for details on the usage of tags.`
    ),
    webhookService: i18n._(t`Select a webhook service.`),
    webhookURL: i18n._(
      t`Webhook services can launch jobs with this workflow job template by making a POST request to this URL.`
    ),
    webhookKey: i18n._(t`Webhook services can use this as a shared secret.`),
    ...
    sourceControlBranch: i18n._(
      t`Branch to use in job run. Project default used if blank. Only allowed if project allow_override field is set to true.`
    ),
    ...
    enableWebhook: i18n._(t`Enable webhook for this template.`),
    concurrentJobs: i18n._(
      t`If enabled, simultaneous runs of this job template will be allowed.`
    ),
    preventInstanceGroupFallback: i18n._(
      t`If enabled, the job template will prevent adding any inventory or organization instance groups to the list of preferred instances groups to run on.\n      Note: If this setting is enabled and you provided an empty list, the global instance groups will be applied.`
    ),
    ...
    enabledOptions: (
      <>
        ...
        <p>{i18n._(t`Webhooks: Enable webhook for this template.`)}</p>
        <p>
          {i18n._(
