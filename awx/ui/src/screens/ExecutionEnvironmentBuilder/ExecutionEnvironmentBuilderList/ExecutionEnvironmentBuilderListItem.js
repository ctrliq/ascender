...
function ExecutionEnvironmentBuilderListItem({
  ...
  const launchBuild = useCallback(async () => {
    try {
      ...
      if (response.status === 201) {
        navigate(`/jobs/build/${response.data.execution_environment_builder_build}`);
      }
    } catch (err) {
      setLaunchError(err);
    } finally {
  ...
  }, [executionEnvironmentBuilder.id, executionEnvironmentBuilder.name, navigate]);
...
ExecutionEnvironmentBuilderListItem.propTypes = {
  executionEnvironmentBuilder: object.isRequired,
