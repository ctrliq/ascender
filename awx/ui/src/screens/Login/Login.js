...
function AWXLogin({ alt, isAuthenticated }) {
  ...
  if (authError?.response?.status === 401) {
    ...
    helperText = i18n._(t`There was a problem logging in. Please try again.`);
  }
  ...
  return (
    <Login header={Header} footer={Footer}>
      <LoginMainHeader
        ...
        /*        title={brandName ? i18n._(t`Welcome to ${brandName}!`) : ''} */
        title=""
        /*        subtitle={i18n._(t`Please log in`)} */
        subtitle=""
      />
      <LoginMainBody>
        {isSessionExpired.current ? (
          <Alert
            ...
            isInline
            title={i18n._(
              t`Your session has expired. Please log in to continue where you left off.`
            )}
                ...
                if (authKey === 'oidc') {
                  return (
                    <LoginMainFooterLinksItem
                      ...
                      onClick={setSessionRedirect}
                    >
                      <Tooltip content={i18n._(t`Sign in with OIDC`)}>
