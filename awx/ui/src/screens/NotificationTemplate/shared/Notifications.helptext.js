...
const helpText = (i18n) => ({
  emailRecepients: i18n._(
    msg`Use one email address per line to create a recipient list for this type of notification.`
  ),
  emailTimeout: i18n._(
    msg`The amount of time (in seconds) before the email
        notification stops trying to reach the host and times out. Ranges
        from 1 to 120 seconds.`
  ),
  grafanaUrl: i18n._(
    msg`The base URL of the Grafana server - the
      /api/annotations endpoint will be added automatically to the base
      Grafana URL.`
  ),
  grafanaTags: i18n._(msg`Use one Annotation Tag per line, without commas.`),
  ircTargets: i18n._(
    msg`Use one IRC channel or username per line. The pound
  ...
  slackChannels: (
    <>
      {i18n._(
        msg`One Slack channel per line. The pound symbol (#)
      ...
      <span>{i18n._(msg`for more information.`)}</span>
    </>
  ),
  slackColor: i18n._(
    msg`Specify a notification color. Acceptable colors are hex
        color code (example: #3af or #789abc).`
  ),
  twilioSourcePhoneNumber: i18n._(
    msg`The number associated with the "Messaging
        Service" in Twilio with the format +18005550199.`
  ),
  twilioDestinationNumbers: i18n._(
    msg`Use one phone number per line to specify where to
  ...
  webhookHeaders: i18n._(
    msg`Specify HTTP Headers in JSON format. Refer to
      the Ansible Controller documentation for example syntax.`
  ),
  emailOptions: (
    <>
      {i18n._(msg`See Django`)}{' '}
      <a
