...
function Organization({ setBreadcrumb, me }: OrganizationProps) {
  const location = useLocation();
  const { id: organizationId } = useParams() as { id: string };
      ...
      <PageSection hasBodyWrapper={false}>
        <Card>
          <ContentError error={organizationError}>
            {(organizationError as DetailedError).response?.status === 404 && (
              <span>
