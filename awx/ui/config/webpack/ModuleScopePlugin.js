...
const colors = require('../devUtils/colors');

// Prevents application code from importing files outside of src/ (other than
// node_modules and an explicit allow-list), since only src/ is run through
// babel.
      ...
      (request, contextResolver, callback) => {
        ...
        if (
          ...
          const scopeError = new Error(
            `You attempted to import ${colors.cyan(
              request.__innerRequest_request
            )} which falls outside of the project ${colors.cyan(
              'src/'
            )} directory. ` +
              `Relative imports outside of ${colors.cyan(
                'src/'
              )} are not supported.` +
              os.EOL +
              `You can either move it inside ${colors.cyan(
                'src/'
              )}, or add a symlink to it from project's ${colors.cyan(
                'node_modules/'
              )}.`
          );
