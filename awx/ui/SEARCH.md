...
The REST API is coupled with the qs util through the `paramsSerializer` (the `encodeQueryString` function), due to the fact we need to support the array for duplicate key values in the object representation of the params to pass to the get request. This is done where the HTTP client is configured in the Base.ts file, so all requests and request types should support our array syntax for duplicate keys automatically.
