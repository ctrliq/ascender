...
.. glossary::
    ...
    Credentials
        Authentication details that may be utilized by Ascender to launch jobs against machines, to synchronize with inventory sources, and to import project content from a version control system.
    ...
    External Credential Type
        A managed credential type for Ascender used for authenticating with a secret management system.
    ...
    Host
        A system managed by Ascender, which may include a physical, virtual, cloud-based server, or other device. Typically an operating system instance. Hosts are contained in Inventory. Sometimes referred to as a "node".
    ...
    Job
        One of many background tasks launched by Ascender, this is usually the instantiation of a Job Template; the launch of an Ansible playbook. Other types of jobs include inventory imports, project synchronizations from source control, or administrative cleanup actions.
    ...
    JSON
        Ansible and Ascender use JSON for return data from remote modules. This allows modules to be written in any language, not just Python.
    ...
    Organization
        A logical collection of Users, Teams, Projects, and Inventories. The highest level in the Ascender object hierarchy is the Organization.

    Organization Administrator
        An Ascender user with the rights to modify the Organization's membership and settings, including making new users and projects within that organization. An organization admin can also grant permissions to other users within the organization.

    Permissions
        The set of privileges assigned to Users and Teams that provide the ability to read, modify, and administer Projects, Inventories, and other Ascender objects.
    ...
    Project
        A logical collection of Ansible playbooks, represented in Ascender.

    Roles
        Roles are units of organization in Ansible and Ascender. Assigning a role to a group of hosts (or a set of groups, or host patterns, etc.) implies that they should implement a specific behavior. A role may include applying certain variable values, certain tasks, and certain handlers--or just one or more of these things. Because of the file structure associated with a role, roles become redistributable units that allow you to share behavior among playbooks--or even with other users.
    ...
    Superuser
        An admin of the Ascender server who has permission to edit any object in the system, whether associated to any organization. Superusers can create organizations and other superusers.
