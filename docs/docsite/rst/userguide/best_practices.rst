...
.. note::
    Playbooks should not use the ``vars_prompt`` feature, as Ascender does not interactively allow for ``vars_prompt`` questions. If you must use ``vars_prompt``, refer to and make use of the :ref:`ug_surveys` functionality.

.. note::
    Playbooks should not use the ``pause`` feature of Ansible without a timeout, as Ascender does not allow for interactively cancelling a pause. If you must use ``pause``, ensure that you set a timeout.
...
``host_vars/``. If you use dynamic inventory sources, Ascender can sync
