...
1. In the Prometheus config file (typically ``prometheus.yml``), specify a ``<token_value>``, a valid user/password for an Ascender user you have created, and a ``<ascender_host>``. 
 ...
 Using an OAuth2 Token, created at the ``/api/v2/tokens`` endpoint to authenticate prometheus with Ascender, the following example provides a valid scrape config if the URL for your Ascender's metrics endpoint was ``https://ascender_host:443/metrics``.
