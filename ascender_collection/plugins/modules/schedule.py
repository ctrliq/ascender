...
- name: Build the same schedule using the rrule plugin
  schedule:
    ...
    state: present
    unified_job_template: "Demo Job Template"
    rrule: "{{ query('ctrliq.ascender.schedule_rrule', 'week', start_date='2019-12-19 13:05:51') }}"
  register: result
...
- name: Build a complex schedule for every day except sunday using the rruleset plugin
  schedule:
    ...
    state: present
    unified_job_template: "Demo Job Template"
    rrule: "{{ query(ctrliq.ascender.schedule_rruleset, '2022-04-30 10:30:45', rules=rrules, timezone='UTC' ) }}"
  vars:
    rrules:
      - frequency: 'day'
        every: 1
